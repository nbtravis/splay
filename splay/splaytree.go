...
// Returns value associated with key if it exists, or nil if not
...
// Creates a (key, value) Node in the tree and splays it. Returns error which is
// only non-nil if the key already exists
...
// Deletes Node corresponding to key from tree then splays parent. Returns an
// error which is only non-nil if no Node with the key exists
...
// Completes splay steps on n until it reaches the root
...
// Prints a directory-looking picture of ST to stdout
